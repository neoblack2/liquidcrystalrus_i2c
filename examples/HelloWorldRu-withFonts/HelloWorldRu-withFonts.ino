...
void print_rus(char *str) { // процедура вывода русских букв
  ...
  for( i=0; i<strlen(str); i++ ) { 
    if( byte(str[i]) == 208 or byte(str[i]) == 209 ) continue; // 208 209 игнор, первый байт UTF8
    for(j=0; j < lcount; j++) if( byte(str[i]) == byte(rus_letters[j]) ) {lcd.write(trans_letters[j]); break;}
  }
}
